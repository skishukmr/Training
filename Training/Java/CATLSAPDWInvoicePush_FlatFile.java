...
/*******************************************************************************************************************************************
	...
	ChangeLog:
	Date		Name		     	History
	21/02/12  	Abhishek Kumar     	Creation of Invoice Push Flat files for Non Mach1
...
public class CATLSAPDWInvoicePush_FlatFile extends ScheduledTask
{
    ...
    private String classname="CATLSAPDWInvoicePush_FlatFile";
    private String flatFilePath = "/msc/arb9r1/downstream/catdata/DW/MSC_DW_INVOICE_PUSH_LSAP.txt";
	...
	{
	Log.customer.debug("%s::Start of CATLSAPDWInvoicePush_FlatFile task ...",classname);
	...
	try {
			...
			archiveFileDataPath = "/msc/arb9r1/downstream/catdata/DW/archive/MSC_DW_INVOICE_PUSH_LSAP_ARCHIVE." + fileExtDateTime + ".txt";
			Log.customer.debug("%s::archiveFileDataPath:",classname);

			Log.customer.debug("CATLSAPDWInvoicePush_FlatFile:Starting Copying the flat file to Archive ");
			CATFaltFileUtil.copyFile(flatFilePath, archiveFileDataPath);
			Log.customer.debug("CATLSAPDWInvoicePush_FlatFile:Completed Copying the flat file to Archive ");
			...
			if (!out_FlatFile.exists()) {
				...
				//mailSubject = "CATLSAPDWInvoicePush_FlatFile Task Completion Status - Completed Successfully";
				try
				{
					isHeader = false;
...
//										 Comment Added by Majid - No need to write into file
									    ...
									    if ( source1.equalsIgnoreCase("CGM") ){
											irRecfacility = "GM";
										...
										Log.customer.debug("CATLSAPDWInvoicePush_FlatFile: SupInvNumber: "+ SupInvNumber);
										int invleng = SupInvNumber.length();
										if (invleng > 33)
										{	
											String SupInvNumber1 = getFormatattedTxt(SupInvNumber);
											int invleng1 = SupInvNumber1.length();
											Log.customer.debug("CATLSAPDWInvoicePush_FlatFile: invleng1: "+ invleng1);
											String FormattedirUniqueName = getFormatattedTxt(irUniqueName, invleng1);
											Log.customer.debug("CATLSAPDWInvoicePush_FlatFile: FormattedirUniqueName: "+ FormattedirUniqueName);
										...
										//Writing Discount Amount-11
											...
											if (IrLineDescritpiton!=null){
												...
												descDescription = (String)IrLineDescritpiton.getFieldValue("Description");
...
//													 IF CAPSUnitOfMeasure = Enpty  THEN LineItems.Description.UnitOfMeasure.UniqueName
											 ...
											 if ( IrLineItem2.getDottedFieldValue("Description.UnitOfMeasure") != null){
												...
												int partitionNumber_6 = invrecon.getPartitionNumber();
												Log.customer.debug("%s::partiton number 5th place:%s",classname,partitionNumber_6);
												if (partitionNumber_6==6){
														...
														//if (irSplitaccSize > 0){
															Log.customer.debug ("%s::getting accounting facility",classname);
															   ...
															   if(source.equals("CGM"))
																{
																	Log.customer.debug("DW PO Push : Inside CGM "+source);
																	...
																	{
																		Log.customer.debug("DW PO Push : Inside CGM : WBSElementText is null ");
																		outPW_FlatFile.write("~|");
																	}
															   ...
															   Log.customer.debug("irBuyercode");
																...
																isAdHocBoolean = true;
																isAdHoc = null;
																if (IrLineItem2.getDottedFieldValue("IsAdHoc") != null) {
																	isAdHoc = (Boolean) IrLineItem2.getDottedFieldValue("IsAdHoc");
				...
				message.append("\n");
				message.append("CATLSAPDWInvoicePush_FlatFile Task Failed - Exception details below");
				message.append("\n");
				message.append(e.toString());
				//mailSubject = "CATLSAPDWInvoicePush_FlatFile Task Failed";
...
*/
				try
				{
					Log.customer.debug("CATLSAPDWInvoicePush_FlatFile:Changing file permission of Data file.");
					Runtime.getRuntime().exec("chmod 666 " + flatFilePath);
					Log.customer.debug("CATLSAPDWInvoicePush_FlatFile:Changed file permission of Data file.");
	...
	private String getFormatattedTxt(String inputTxt, int txtLength) {
		...
		Log.customer.debug("CATLSAPDWInvoicePush_FlatFile: int temp  " + temp);

		inputTxt = inputTxt.substring(rellength, fulllength);

		formattedTxt = "IRL-" + inputTxt;
		Log.customer.debug("CATLSAPDWInvoicePush_FlatFile: formattedTxt " + formattedTxt);
		return formattedTxt;
	...
	private String getFormatattedTxt(String inputTxt2) {
		...
		Log.customer.debug("CATLSAPDWInvoicePush_FlatFile: inputTxt2 " + inputTxt2);
		return inputTxt2;

		}
	//End: Q4 2013 - RSD117 - FDD 2/TDD 2

	public CATLSAPDWInvoicePush_FlatFile(){
	}
}